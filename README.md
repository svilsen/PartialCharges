...
-   `02-extractfeatures.R`: A script for identifying partial charges and
    ...
    profiles (i.e., `ROOT` data-folder). Lastly, the script defines the
    relevant features, randomly split the data into training and
    validation-sets, and normalise the features (based solely on the
    training-set). NB: the script defines the features extracted during
    partial charging from within the interval,
...
-   `03-trainmodels.R`: A script used to train models using the
    training-set. NB: the script depends on the features included in
    each of the four segments.

-   `04-trainrandommodels.R`: A script used to repeatedly train models
    using randomly chosen partial charges from the training-set. NB: the
    script depends on the features included in each of the four
    segments.

-   `05-visualisemodels.R`: A script used to visualise the trained
    models, and determine feature importance for each model.
