...
if (make_figures) {
    ...
    mlr_error_tibble <- readRDS("Files/mlr_model_tibble.Rds") |> 
        mutate(
            ...
            Profile = ifelse(Profile != "Forklifts", "WLTC", "Forklifts")
        )
    ...
    pdf("Figures/comparing_methods_NOFEC.pdf", height = 12, width = 20)
    avg_error_tibble |> 
        ggplot(aes(x = FEC)) + 
    ...
    pdf("Figures/comparing_methods_one_to_one_NOFEC.pdf", height = 12, width = 20)
    avg_error_tibble |> 
        ggplot(aes(x = 100*D)) + 
    ...
    pdf("Figures/comparing_methods_difference_NOFEC.pdf", height = 12, width = 20)
    avg_error_tibble |> 
        ggplot(aes(x = 100 * D)) + 
    ...
    pdf("Figures/comparing_methods_density_NOFEC.pdf", height = 12, width = 20)
    total_error_tibble |> 
    ...
    print(xtable::xtable(total_error_table |> filter(Profile == "Forklifts") |> select(-Profile, -Prior) |> bind_cols(total_error_table |> filter(Profile == "WLTC") |> select(-Model, -Profile, -Prior))), include.rownames = FALSE)
}
...
if (create_vi) {
    ...
    for (i in seq_along(model_levels)) {
        ...
        if (models[i] == "RF") {
            ...
            predict_function <- function(object, newdata) {
                x <- newdata |> (\(x) model.matrix(~ -1 + .*., data = x))()
                predict(object, newdata = x)[, 1]
            }
        } else if (models[i] == "NN") {
            predict_function <- function(object, newdata) {
        ...
        saveRDS(variable_importance_i, paste0(files_dir, "/variable_importance_", tolower(models[i]), "_NOFEC.Rds"))
    ...
    for (i in seq_along(models)) {
        variable_importance_i <- readRDS(paste0(files_dir, "/variable_importance_", tolower(models[i]), "_NOFEC.Rds"))
    ...
    saveRDS(variable_importance, paste0(files_dir, "/variable_importance_NOFEC.Rds"))
}

#### Creating figures for variable importance ----
if (make_figures) {
    #### Feature importance plots (without FEC) ----
    ...
    variable_importance <- readRDS(paste0(files_dir, "/variable_importance_NOFEC.Rds")) |> 
        mutate(
            Variable = str_replace_all(Variable, "_", " "), 
            Variable = factor(Variable, levels = rev(str_replace_all(c(window_capacity_features[c(15, 10:14, 1:9, 16, 17)], window_prior_capacity_features[c(2, 1, 11:15, 3:10)], window_prior_features[c(1, 8:13, 2:7, 14)]), "_", " ")))
        )
    
    pdf("Figures/comparing_methods_vi_nn_NOFEC.pdf", height = 10, width = 15)
    variable_importance |> 
        filter(Model == "SVR") |> 
    ...
    pdf("Figures/comparing_methods_vi_REM_NOFEC.pdf", height = 20, width = 15)
    variable_importance |> 
        filter(Model != "SVR") |> 
        ggplot(aes(x = Variable, y = Importance)) + 
        geom_col(aes(fill = Importance)) + 
    ...
    pdf("Figures/comparing_methods_vi_NOFEC.pdf", height = 22, width = 15)
    variable_importance |> 
        ggplot(aes(x = Variable, y = Importance)) + 
        geom_col(aes(fill = Importance)) + 
        facet_grid(Model ~ Prior) + 
    ...
    #### Feature importance plots (with FEC) ----
    window_capacity_features <- c(
        "StartVoltage",
        "DeltaVoltage",
        "Voltage_Average",
    ...
    pdf("Figures/comparing_methods_vi_REM.pdf", height = 20, width = 15)
    variable_importance |> 
        filter(Model != "SVR") |> 
        ggplot(aes(x = Variable, y = Importance)) + 
        geom_col(aes(fill = Importance)) + 
    ...
    pdf("Figures/comparing_methods_vi.pdf", height = 22, width = 15)
    variable_importance |> 
        ggplot(aes(x = Variable, y = Importance)) + 
        geom_col(aes(fill = Importance)) + 
        facet_grid(Model ~ Prior) + 
...
#### Creating figures for random number of partial charges ----
if (make_figures) {
    prior_levels <- c("Without prior information", "Without prior charge information", "Without prior discharge information", "With prior information")
    type_levels <- c("Average weights", "Linear weights", "Exponential weights")
    ...
    full_error_tibble <- mlr_error_tibble |> 
        bind_rows(rf_error_tibble) |> 
        bind_rows(svr_error_tibble) |> 
        bind_rows(nn_error_tibble) |> 
        filter(TYPE == "Average weights") |> 
        filter(Data == "Validation") |> 
        mutate(
            ML = factor(Model, levels = c("MLR", "SVR", "RF", "NN"))
        ) |> 
        group_by(ML, Prior) |> 
        summarise(
            MAE = 100 * mean(abs(DHAT - D)), 
            .groups = "drop"
        )
        
    ####
    mlr_error_tibble <- readRDS("Files/mlr_randommodel_tibble.Rds") 
    rf_error_tibble <- readRDS("Files/rf_randommodel_tibble.Rds")
    svr_error_tibble <- readRDS("Files/svr_randommodel_tibble.Rds")
    nn_error_tibble <- readRDS("Files/nn_randommodel_tibble.Rds")
    
    random_error_tibble <- mlr_error_tibble |> 
        bind_rows(rf_error_tibble) |> 
        bind_rows(svr_error_tibble) |> 
        bind_rows(nn_error_tibble) |> 
        filter(Data == "Validation") |> 
        mutate(
            ML = factor(ML, levels = c("MLR", "SVR", "RF", "NN")),
            Prior = factor(Prior, levels = prior_levels)
        ) |> 
        group_by(ML, Limit, Model, Prior) |> 
        summarise(
            MAE = 100 * mean(abs(DHAT - D)), 
            .groups = "drop"
        )
    
    pdf("Figures/comparing_methods_randomcharges_NOFEC.pdf", height = 12, width = 20)
    random_error_tibble |> 
        ggplot(aes(x = Limit, y = MAE)) + 
        geom_boxplot(aes(group = Limit)) + 
        geom_hline(data = full_error_tibble, aes(yintercept = MAE), col = "red") + 
        facet_grid(ML ~ Prior) + 
        labs(x = "Number of partial charges", y = "MAE [%]", colour = "") + 
    ...
    pdf("Figures/comparing_methods_randomcharges_I_NOFEC.pdf", height = 7, width = 18)
    random_error_tibble |> 
        filter(ML == "MLR") |> 
        ggplot(aes(x = Limit, y = MAE)) + 
        geom_boxplot(aes(group = Limit)) + 
        geom_hline(data = full_error_tibble |> filter(ML == "MLR") , aes(yintercept = MAE), col = "red") + 
        facet_grid(ML ~ Prior) + 
        labs(x = "Number of partial charges", y = "MAE [%]", colour = "") + 
        scale_y_continuous(transform = "log10") + 
        theme_bw(base_size = 20) + 
    ...
    random_error_tibble |> 
        ...
        ggplot(aes(x = Limit, y = MAE)) + 
        geom_boxplot(aes(group = Limit)) + 
        geom_hline(data = full_error_tibble |> filter(ML == "SVR") , aes(yintercept = MAE), col = "red") + 
        facet_grid(ML ~ Prior) + 
        labs(x = "Number of partial charges", y = "MAE [%]", colour = "") + 
        scale_y_continuous(transform = "log10") + 
        theme_bw(base_size = 20) + 
    ...
    random_error_tibble |> 
        ...
        ggplot(aes(x = Limit, y = MAE)) + 
        geom_boxplot(aes(group = Limit)) + 
        geom_hline(data = full_error_tibble |> filter(ML == "RF") , aes(yintercept = MAE), col = "red") + 
        facet_grid(ML ~ Prior) + 
        labs(x = "Number of partial charges", y = "MAE [%]", colour = "") + 
        scale_y_continuous(transform = "log10") + 
        theme_bw(base_size = 20) + 
    ...
    random_error_tibble |> 
        ...
        ggplot(aes(x = Limit, y = MAE)) + 
        geom_boxplot(aes(group = Limit)) + 
        geom_hline(data = full_error_tibble |> filter(ML == "NN") , aes(yintercept = MAE), col = "red") + 
        facet_grid(ML ~ Prior) + 
        labs(x = "Number of partial charges", y = "MAE [%]", colour = "") + 
        scale_y_continuous(transform = "log10") + 
        theme_bw(base_size = 20) + 
